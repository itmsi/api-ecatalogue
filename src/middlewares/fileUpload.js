...
// Configure multer for item catalog axle uploads (file_foto + file_csv)
const itemCatalogAxleUploadStorage = multer({
  storage: storage,
  limits: {
    fileSize: 10 * 1024 * 1024, // 10MB limit
    files: 2 // file_foto + file_csv
...
// Middleware for item catalog axle with file_foto and file_csv
const uploadItemCatalogAxleFields = itemCatalogAxleUploadStorage.fields([
  { name: 'file_foto', maxCount: 1 },
  { name: 'file_csv', maxCount: 1 }
]);

// Middleware wrapper to handle item catalog axle upload errors
const handleItemCatalogAxleUpload = (req, res, next) => {
  uploadItemCatalogAxleFields(req, res, (err) => {
    if (err instanceof multer.MulterError) {
      if (err.code === 'LIMIT_FILE_SIZE') {
        return res.status(400).json({
          success: false,
...
// Configure multer for item catalog steering uploads (file_foto + file_csv)
const itemCatalogSteeringUploadStorage = multer({
  storage: storage,
  limits: {
...
// Middleware for item catalog steering with file_foto and file_csv
const uploadItemCatalogSteeringFields = itemCatalogSteeringUploadStorage.fields([
  { name: 'file_foto', maxCount: 1 },
  { name: 'file_csv', maxCount: 1 }
]);

// Middleware wrapper to handle item catalog steering upload errors
const handleItemCatalogSteeringUpload = (req, res, next) => {
  uploadItemCatalogSteeringFields(req, res, (err) => {
    if (err instanceof multer.MulterError) {
      if (err.code === 'LIMIT_FILE_SIZE') {
        return res.status(400).json({
          success: false,
