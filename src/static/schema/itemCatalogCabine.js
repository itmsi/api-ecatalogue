...
const itemCatalogCabineSchemas = {
  ...
  ItemCatalogCabineMultipartInput: {
    type: 'object',
    required: ['name_pdf'],
    properties: {
      name_pdf: {
        ...
        example: 'Cabine Catalog 2024'
      },
      cabine_id: {
        type: 'string',
        format: 'uuid',
        description: 'Cabine ID (optional). Berlaku untuk SEMUA items',
        example: '1cd67a84-c5d1-46ff-b5c2-f85a70512227'
      },
      type_cabine_id: {
        type: 'string',
        format: 'uuid',
        description: 'Type Cabine ID (optional). Berlaku untuk SEMUA items',
      ...
      file_csv: {
        type: 'string',
        format: 'binary',
        description: 'File CSV untuk import data (required jika use_csv = true). Max 10MB. CSV tidak perlu kolom cabine_id dan type_cabine_id.'
      },
      use_csv: {
        type: 'boolean',
      ...
      data_items: {
        type: 'string',
        description: 'JSON string array of items (required jika use_csv = false). Tidak perlu include cabine_id dan type_cabine_id di setiap item. Format: [{"target_id":"T001","diagram_serial_number":"DSN001",...}]',
        example: '[{"target_id":"T001","diagram_serial_number":"DSN001","part_number":"PN001","catalog_item_name_en":"Cabine Part 1","catalog_item_name_ch":"驾驶室部件1","description":"Description","quantity":10}]'
      }
    }
  },
  ItemCatalogCabineDetail: {
    type: 'object',
    properties: {
      name_pdf: { type: 'string' },
      master_pdf_id: { type: 'string', format: 'uuid' },
