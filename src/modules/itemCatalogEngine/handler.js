...
/**
 * Validate CSV row data for engine items
 */
const validateCsvRow = (row, rowIndex) => {
  ...
  // Tidak ada validasi required untuk engine items
...
const processCsvToDataItems = async (csvBuffer) => {
  ...
  // Convert CSV data ke format data_items (tanpa engine_id dan type_engine_id di row level)
...
const create = async (req, res) => {
  ...
  try {
    const { name_pdf, engine_id, type_engine_id, use_csv } = req.body;
    ...
    console.log('type_engine_id:', type_engine_id);
    console.log('dataItems[0]:', JSON.stringify(dataItems[0], null, 2));
    
    // Create items dengan transaction (pass engine_id dan type_engine_id dari root level)
...
const update = async (req, res) => {
  ...
  try {
    ...
    // Update items dengan transaction (pass engine_id dan type_engine_id dari root level)
