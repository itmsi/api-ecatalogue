...
const TABLE_NAME = 'item_catalog_axles';
...
const findAll = async (filters) => {
  ...
  let query = db(TABLE_NAME)
    ...
    .leftJoin('axels', `${TABLE_NAME}.axel_id`, 'axels.axel_id')
    .leftJoin('type_axels', `${TABLE_NAME}.type_axel_id`, 'type_axels.type_axel_id')
...
const findById = async (id) => {
  const data = await db(TABLE_NAME)
    ...
    .leftJoin('axels', `${TABLE_NAME}.axel_id`, 'axels.axel_id')
    .leftJoin('type_axels', `${TABLE_NAME}.type_axel_id`, 'type_axels.type_axel_id')
    .where({ [`${TABLE_NAME}.item_catalog_axle_id`]: id, [`${TABLE_NAME}.is_delete`]: false })
    .whereNull(`${TABLE_NAME}.deleted_at`)
    .first();
...
const create = async (namePdf, dataItems, userId) => {
  ...
  const itemsToInsert = dataItems.map(item => ({
    ...
    axel_id: item.axel_id || null,
    type_axel_id: item.type_axel_id || null,
 ...
 * Jika kombinasi master_pdf_id + axel_id + type_axel_id + target_id sudah ada, maka UPDATE
 * Jika belum ada, maka INSERT
 */
const createWithTransaction = async (trx, namePdf, dataItems, userId, fileFotoUrl = null, axelId = null, typeAxelId = null) => {
  ...
  console.log('=== REPOSITORY CREATE: axelId:', axelId);
  console.log('=== REPOSITORY CREATE: typeAxelId:', typeAxelId);
  ...
  for (const item of dataItems) {
    ...
    // Add conditions for axel_id, type_axel_id, target_id only if they are not null
    if (axelId) whereClause.axel_id = axelId;
    if (typeAxelId) whereClause.type_axel_id = typeAxelId;
    ...
    if (existing) {
      ...
      const [updated] = await trx(TABLE_NAME)
        .where({ item_catalog_axle_id: existing.item_catalog_axle_id })
        .update({
          ...updateData,
          updated_at: trx.fn.now(),
          updated_by: userId
...
const update = async (id, namePdf, dataItems, userId) => {
  ...
  const existingItem = await db(TABLE_NAME)
    .where({ item_catalog_axle_id: id, is_delete: false })
    .whereNull('deleted_at')
    .first();
 ...
 * Jika belum ada, maka INSERT
 */
const updateWithTransaction = async (trx, id, namePdf, dataItems, userId, fileFotoUrl = null, axelId = null, typeAxelId = null) => {
  // Cek apakah item dengan id tersebut ada
  ...
  console.log('=== REPOSITORY UPDATE: axelId:', axelId);
  console.log('=== REPOSITORY UPDATE: typeAxelId:', typeAxelId);
