...
/**
 * Validate CSV row data for axle items
 */
const validateCsvRow = (row, rowIndex) => {
  ...
  // Tidak ada validasi required untuk axle items
...
const processCsvToDataItems = async (csvBuffer) => {
  ...
  // Convert CSV data ke format data_items (tanpa axel_id dan type_axel_id di row level)
...
const create = async (req, res) => {
  ...
  try {
    const { name_pdf, axel_id, type_axel_id, use_csv } = req.body;
    ...
    console.log('axel_id:', axel_id);
    console.log('type_axel_id:', type_axel_id);
    console.log('dataItems[0]:', JSON.stringify(dataItems[0], null, 2));
    
    // Create items dengan transaction (pass axel_id dan type_axel_id dari root level)
...
const update = async (req, res) => {
  ...
  try {
    ...
    // Update items dengan transaction (pass axel_id dan type_axel_id dari root level)
