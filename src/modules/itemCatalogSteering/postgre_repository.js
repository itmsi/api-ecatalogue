...
const TABLE_NAME = 'item_catalog_steerings';
...
const findAll = async (filters) => {
  ...
  let query = db(TABLE_NAME)
    ...
    .leftJoin('steerings', `${TABLE_NAME}.steering_id`, 'steerings.steering_id')
    .leftJoin('type_steerings', `${TABLE_NAME}.type_steering_id`, 'type_steerings.type_steering_id')
...
const findById = async (id) => {
  const data = await db(TABLE_NAME)
    ...
    .leftJoin('steerings', `${TABLE_NAME}.steering_id`, 'steerings.steering_id')
    .leftJoin('type_steerings', `${TABLE_NAME}.type_steering_id`, 'type_steerings.type_steering_id')
    .where({ [`${TABLE_NAME}.item_catalog_steering_id`]: id, [`${TABLE_NAME}.is_delete`]: false })
    .whereNull(`${TABLE_NAME}.deleted_at`)
    .first();
...
const create = async (namePdf, dataItems, userId) => {
  ...
  const itemsToInsert = dataItems.map(item => ({
    ...
    steering_id: item.steering_id || null,
    type_steering_id: item.type_steering_id || null,
...
const update = async (id, namePdf, dataItems, userId) => {
  ...
  const existingItem = await db(TABLE_NAME)
    .where({ item_catalog_steering_id: id, is_delete: false })
    .whereNull('deleted_at')
    .first();
 ...
 * Jika kombinasi master_pdf_id + steering_id + type_steering_id + target_id sudah ada, maka UPDATE
 * Jika belum ada, maka INSERT
 */
const createWithTransaction = async (trx, namePdf, dataItems, userId, fileFotoUrl = null, steeringId = null, typeSteeringId = null) => {
  ...
  console.log('=== REPOSITORY CREATE: steeringId:', steeringId);
  console.log('=== REPOSITORY CREATE: typeSteeringId:', typeSteeringId);
  ...
  for (const item of dataItems) {
    ...
    const itemData = {
      master_pdf_id: masterPdfId,
      steering_id: steeringId || null,
      type_steering_id: typeSteeringId || null,
      target_id: item.target_id || null,
      diagram_serial_number: item.diagram_serial_number || null,
      part_number: item.part_number || null,
      catalog_item_name_en: item.catalog_item_name_en || null,
    ...
    // Add conditions for steering_id, type_steering_id, target_id only if they are not null
    if (steeringId) whereClause.steering_id = steeringId;
    if (typeSteeringId) whereClause.type_steering_id = typeSteeringId;
    ...
    if (existing) {
      ...
      const [updated] = await trx(TABLE_NAME)
        .where({ item_catalog_steering_id: existing.item_catalog_steering_id })
        .update({
          ...updateData,
          updated_at: trx.fn.now(),
          updated_by: userId
 ...
 * Jika belum ada, maka INSERT
 */
const updateWithTransaction = async (trx, id, namePdf, dataItems, userId, fileFotoUrl = null, steeringId = null, typeSteeringId = null) => {
  // Cek apakah item dengan id tersebut ada
  ...
  console.log('=== REPOSITORY UPDATE: steeringId:', steeringId);
  console.log('=== REPOSITORY UPDATE: typeSteeringId:', typeSteeringId);
