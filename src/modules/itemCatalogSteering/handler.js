...
/**
 * Validate CSV row data for steering items
 */
const validateCsvRow = (row, rowIndex) => {
  ...
  // Tidak ada validasi required untuk steering items
...
const processCsvToDataItems = async (csvBuffer) => {
  ...
  // Convert CSV data ke format data_items (tanpa steering_id dan type_steering_id di row level)
...
const create = async (req, res) => {
  ...
  try {
    const { name_pdf, steering_id, type_steering_id, use_csv } = req.body;
    ...
    console.log('steering_id:', steering_id);
    console.log('type_steering_id:', type_steering_id);
    console.log('dataItems[0]:', JSON.stringify(dataItems[0], null, 2));
    
    // Create items dengan transaction (pass steering_id dan type_steering_id dari root level)
...
const update = async (req, res) => {
  ...
  try {
    ...
    // Update items dengan transaction (pass steering_id dan type_steering_id dari root level)
