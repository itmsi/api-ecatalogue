...
/**
 * Validate CSV row data for all catalog items
 */
const validateCsvRow = (row, rowIndex) => {
  ...
  // Tidak ada validasi required untuk catalog items
...
const processCsvToDataItems = async (csvBuffer) => {
  ...
  // Convert CSV data ke format data_items
...
const getAll = async (req, res) => {
  try {
    const filters = {
      ...
      master_catalog: req.body.master_catalog || null
    };
...
const create = async (req, res) => {
  ...
  try {
    ...
    console.log('type_category_id:', type_category_id);
    console.log('dataItems[0]:', JSON.stringify(dataItems[0], null, 2));
    
    // Create items dengan transaction
...
const update = async (req, res) => {
  ...
  try {
    ...
    // Update items dengan transaction
