...
const TABLE_NAME = 'item_catalog_transmissions';
...
const findAll = async (filters) => {
  ...
  let query = db(TABLE_NAME)
    ...
    .leftJoin('transmissions', `${TABLE_NAME}.transmission_id`, 'transmissions.transmission_id')
    .leftJoin('type_transmissions', `${TABLE_NAME}.type_transmission_id`, 'type_transmissions.type_transmission_id')
...
const findById = async (id) => {
  const data = await db(TABLE_NAME)
    ...
    .leftJoin('transmissions', `${TABLE_NAME}.transmission_id`, 'transmissions.transmission_id')
    .leftJoin('type_transmissions', `${TABLE_NAME}.type_transmission_id`, 'type_transmissions.type_transmission_id')
    .where({ [`${TABLE_NAME}.item_catalog_transmission_id`]: id, [`${TABLE_NAME}.is_delete`]: false })
    .whereNull(`${TABLE_NAME}.deleted_at`)
    .first();
...
const create = async (namePdf, dataItems, userId) => {
  ...
  const itemsToInsert = dataItems.map(item => ({
    ...
    transmission_id: item.transmission_id || null,
    type_transmission_id: item.type_transmission_id || null,
 ...
 * Jika kombinasi master_pdf_id + transmission_id + type_transmission_id + target_id sudah ada, maka UPDATE
 * Jika belum ada, maka INSERT
 */
const createWithTransaction = async (trx, namePdf, dataItems, userId, fileFotoUrl = null, transmissionId = null, typeTransmissionId = null) => {
  ...
  console.log('=== REPOSITORY CREATE: transmissionId:', transmissionId);
  console.log('=== REPOSITORY CREATE: typeTransmissionId:', typeTransmissionId);
  ...
  for (const item of dataItems) {
    ...
    const itemData = {
      ...
      type_transmission_id: typeTransmissionId || null,
      target_id: item.target_id || null,
      diagram_serial_number: item.diagram_serial_number || null,
      part_number: item.part_number || null,
      catalog_item_name_en: item.catalog_item_name_en || null,
    ...
    // Add conditions for transmission_id, type_transmission_id, target_id only if they are not null
    if (transmissionId) whereClause.transmission_id = transmissionId;
    if (typeTransmissionId) whereClause.type_transmission_id = typeTransmissionId;
    ...
    if (existing) {
      ...
      const [updated] = await trx(TABLE_NAME)
        .where({ item_catalog_transmission_id: existing.item_catalog_transmission_id })
        .update({
          ...updateData,
          updated_at: trx.fn.now(),
          updated_by: userId
...
const update = async (id, namePdf, dataItems, userId) => {
  ...
  const existingItem = await db(TABLE_NAME)
    .where({ item_catalog_transmission_id: id, is_delete: false })
    .whereNull('deleted_at')
    .first();
 ...
 * Jika belum ada, maka INSERT
 */
const updateWithTransaction = async (trx, id, namePdf, dataItems, userId, fileFotoUrl = null, transmissionId = null, typeTransmissionId = null) => {
  // Cek apakah item dengan id tersebut ada
  ...
  console.log('=== REPOSITORY UPDATE: transmissionId:', transmissionId);
  console.log('=== REPOSITORY UPDATE: typeTransmissionId:', typeTransmissionId);
