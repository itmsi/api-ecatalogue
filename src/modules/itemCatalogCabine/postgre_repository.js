 ...
 * Jika kombinasi master_pdf_id + cabine_id + type_cabine_id + target_id sudah ada, maka UPDATE
 * Jika belum ada, maka INSERT
 */
const createWithTransaction = async (trx, namePdf, dataItems, userId, fileFotoUrl = null, cabineId = null, typeCabineId = null) => {
  ...
  console.log('=== REPOSITORY CREATE: cabineId:', cabineId);
  console.log('=== REPOSITORY CREATE: typeCabineId:', typeCabineId);
  ...
  for (const item of dataItems) {
    ...
    const itemData = {
      master_pdf_id: masterPdfId,
      cabine_id: cabineId || null,
      type_cabine_id: typeCabineId || null,
      target_id: item.target_id || null,
      diagram_serial_number: item.diagram_serial_number || null,
      part_number: item.part_number || null,
      catalog_item_name_en: item.catalog_item_name_en || null,
    ...
    // Add conditions for cabine_id, type_cabine_id, target_id only if they are not null
    if (cabineId) whereClause.cabine_id = cabineId;
    if (typeCabineId) whereClause.type_cabine_id = typeCabineId;
    ...
    if (existing) {
      ...
      const [updated] = await trx(TABLE_NAME)
        .where({ item_catalog_cabine_id: existing.item_catalog_cabine_id })
        .update({
          ...updateData,
          updated_at: trx.fn.now(),
          updated_by: userId
 ...
 * Jika belum ada, maka INSERT
 */
const updateWithTransaction = async (trx, id, namePdf, dataItems, userId, fileFotoUrl = null, cabineId = null, typeCabineId = null) => {
  // Cek apakah item dengan id tersebut ada
  ...
  console.log('=== REPOSITORY UPDATE: cabineId:', cabineId);
  console.log('=== REPOSITORY UPDATE: typeCabineId:', typeCabineId);
